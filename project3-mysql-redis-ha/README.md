...
> 本节按踩坑的先后顺序排列：**7.1~7.7** 是写这套配置时踩到、并已经在仓库代码里修掉的；**7.8** 是实机部署时才暴露出来的（Redis 7.4 + alpine 的兼容性问题）。
>
> 本节偏重"问题本身与解法"；**完整的排查过程**（走了哪些弯路、怎么定位到根因）记在 [`docs/排障实录.md`](../docs/排障实录.md)，两个项目的坑都在里面。
