...
| [`project3-mysql-redis-ha/`](project3-mysql-redis-ha/) | MySQL GTID 一主两从 + Redis 一主两从三哨兵，含故障转移验证 | ✅ **已实机验证** |
| [`docs/排障实录.md`](docs/排障实录.md) | **部署排障实录**：实际踩到的 10 个坑，逐个记录现象、排查过程、根因与解法 | ✅ |
| `docs/images/` | 终端验证截图，共 4 张 | ✅ |

---

## 排障实录

部署过程里踩的坑、定位思路和解决办法，全部记在 [`docs/排障实录.md`](docs/排障实录.md)。

其中三条排查耗时最长，也最能体现"真的动手了"：

| 坑 | 为什么难查 |
|---|---|
| **容器内 DNS 解析失败** | `docker pull` 完全正常，你会误以为网络没问题；换 Maven 源仍失败才暴露真相 |
| **MySQL 配置文件权限 777 被忽略** | MySQL 只打一行 Warning 就正常启动，报错却指向 GTID，实际是整个文件没加载 |
| **Redis 7.4 哨兵解析不了 master 主机名** | 同网络的 redis-slave 用容器名能连主库，证据会误导你排除 DNS 方向 |
